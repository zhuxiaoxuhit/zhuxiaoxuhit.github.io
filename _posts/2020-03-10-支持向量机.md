...
在超平面$w^{T}x_{i}+b=0$确定的情况下，$|w^{T}x_{i}|$能够相对的表示采样点距离超平面的远近，而通过判断$w^{T}x_{i}+b$的符号是否与$y_{i}$一致可以判断分类是否正确。所以可以用$y_{i}(w^{T}x_{i}+b)$代表分类的正确性与确信度。这就是函数间隔的定义：
...
<center>$$k(x_{i},x_{j})= \left \langle \phi(x_{i}),\phi(x_{j}) \right \rangle=\phi(x_{i}^{T}})\phi({x_{j})$$</center>
